...
class DynamicKnowledgeBase:
    def __init__(self, text_embedder, image_model, dim=384):
        # 初始化FAISS索引和文本图像嵌入器
    ...
    def add_image_data(self, image_path):
        ...
        image_input = self.preprocess(image).unsqueeze(0).to('cuda')  # 使用GPU处理图像
        with torch.no_grad():
    ...
    def search(self, query, top_k=5):
        ...
        if isinstance(query, str):
            ...
            query_input = self.preprocess(query).unsqueeze(0).to('cuda')  # 对图像进行处理
            with torch.no_grad():
